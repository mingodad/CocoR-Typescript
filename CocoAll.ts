...
export function sprintf (...argv : any) {
...
//namespace at.jku.ssw.Coco {
    ...
    export class SymInfo { // output attribute of symbols in the ATG
        name : string;
    ...
    export class TabCNode {	// node of list for finding circular productions
        public  left : Symbol;
        public right : Symbol;
    ...
    export class Tab {
        ...
        public  nsName : string | null = null;             // namespace for generated files
            ...
            while (p != null)
            {
                ...
                case Node.nt:
                {
                    if (p.sym!.firstReady)
                    {
                        ...
                        {
                            if (indent.length == 1) this.parser.log(`${indent}=> ${p.sym!.name}:${p.line}:${p.col}:`);
                            this.parser.log(`${indent}-> ${p.sym.name}:${p.sym.line}:${p.sym.col}:`);
                ...
                case Node.wt:
                {
                    if (p.sym!.n == tok) this.parser.log(`${indent}= ${p.sym!.name}:${p.line}:${p.col}:`);
                    break;
                }
                case Node.any:
                {
...
//namespace at.jku.ssw.Coco {
    ...
    export type WriteToHandler = (sender: string, constent : string) => void;
    export class ParserGen {
        ...
        public  WriteParser () : void {
            ...
            if (this.tab.nsName != null && this.tab.nsName.length > 0 && (this.langGen != "js")) this.gen.WriteLine("}");
...
/*//----Start Parser.frame */
export let CocoParserFrame : string = `
/*----------------------------------------------------------------------
Compiler Generator Coco/R,
Copyright (c) 1990, 2004 Hanspeter Moessenboeck, University of Linz
extended by M. Loeberbauer & A. Woess, Univ. of Linz
...
/*//----Start Scanner.frame */
export let CocoScannerFrame : string = `
/*----------------------------------------------------------------------
Compiler Generator Coco/R,
Copyright (c) 1990, 2004 Hanspeter Moessenboeck, University of Linz
extended by M. Loeberbauer & A. Woess, Univ. of Linz
...
export let CocoCopyrightFrame : string = `
/*----------------------------------------------------------------------
Compiler Generator Coco/R,
Copyright (c) 1990, 2004 Hanspeter Moessenboeck, University of Linz
extended by M. Loeberbauer & A. Woess, Univ. of Linz
...
export let CocoParserJsFrame : string = `
/*----------------------------------------------------------------------
Compiler Generator Coco/R,
Copyright (c) 1990, 2004 Hanspeter Moessenboeck, University of Linz
extended by M. Loeberbauer & A. Woess, Univ. of Linz
...
export let CocoScannerJsFrame : string = `
/*----------------------------------------------------------------------
Compiler Generator Coco/R,
Copyright (c) 1990, 2004 Hanspeter Moessenboeck, University of Linz
extended by M. Loeberbauer & A. Woess, Univ. of Linz
...
//namespace at.jku.ssw.Coco {
    ...
    //  Language Generator
    //-----------------------------------------------------------------------------
    export type LangGenSet = "js" | "ts";
    export class LangGen {
...
if(stdScriptArgs.length > 1) {
    ...
    CocoR.CocoParserFrame = std.loadFile("Parser-" + frame_type + ".frame");
    CocoR.CocoScannerFrame = std.loadFile("Scanner-" + frame_type + ".frame");
    CocoR.CocoCopyrightFrame = std.loadFile("Copyright.frame");
    //console.log("====", CocoR.CocoParserFrame, "=====");
    //console.log("====", CocoR.CocoScannerFrame, "=====");
    //console.log("====", CocoR.CocoCopyrightFrame, "=====");

    let scanner = new CocoR.Scanner(grammar, grammar_fname);
    /*
    tok = scanner.Scan()
    while(tok.kind != Scanner.eofSym)
    {
    ...
    */
    ...
    let parser = new CocoR.Parser(scanner);
    parser.trace = new CocoR.StringWriter();
    parser.tab = new CocoR.Tab(parser);
    ...
    parser.dfa = new CocoR.DFA(parser);
    parser.pgen = new CocoR.ParserGen(parser);
